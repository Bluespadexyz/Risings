...
import RaffleRisings from "abis/RaffleRisings.json";
...
const loadActiveRaffles = (dataContract, period, waitingPeriod) => {
  ...
  const ret = dataContract.map((item, index) => {
    ...
    const raffleInfo = Risings_RAFFLES[index];
...
const Raffles = (props) => {
  ...
  const raffleAddress = getContract(chainId, "RaffleRisings");
  ...
  const { data: period } = useSWR(
    ...
    { fetcher: contractFetcher(library, RaffleRisings) }
  );
  ...
  const [lohTicketsNum, setRisingsTicketsNum] = useState(1);
  ...
  const needApproval = lohTicketsNum > parseInt(formatAmount(approvedAmount, 18));

  const onIncreaseBuyNumBtn = () => {
    const max = parseInt(formatAmount(ticketBalance, 18, 0));
    if (max === 0) {
      setRisingsTicketsNum(0);
      return;
    }
    if (lohTicketsNum === max) return;
    setRisingsTicketsNum((prevBuyNum) => prevBuyNum + 1);
  };
  ...
  const onBuyRaffleBtn = (raffleIndex) => {
    ...
    if (max === 0)
      ...
      setRisingsTicketsNum(1);
    setRaffleIndex(raffleIndex);
    setOpenBuyModal(true);
  };

  const approveRisingsTicket = () => {
    if (needApproval) {
      approveTokens({
        setIsApproving,
        library,
  ...
  const onUseRisingsTicketsBtn = () => {
    if (needApproval) {
      approveRisingsTicket();
      return;
    }
    ...
    callContract(chainId, contract, "addTicket", [raffleIndex, lohTicketsNum], {
  ...
  // useEffect(() => {
  //   if (lohTicketsNum > parseInt(formatAmount(approvedAmount, 18)))
    ...
    <Layout>
      <RafflesContainer>
        ...
        <BuyRafflesModal
          ...
          onUseRisingsTicketsBtn={onUseRisingsTicketsBtn}
          isMobile={isMobile}
        />
